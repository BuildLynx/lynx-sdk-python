...
This is the protocol state machine from A2.1 section 7.7. It does not start
threads, publish MQTT, or know about Poll vs Stream; callers map its outcomes
onto status and Notices. Poll never uses it, because Poll does not set
status.command.
"""
...
class CommandMachine:
    ...
    def try_begin(self, command: str, payload: Optional[Dict[str, Any]] = None) -> bool:
        ...
        Returns:
            True if the machine was idle and is now active, False if rejected.
        """
        with self._lock:
